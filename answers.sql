...
SELECT orders.id, status, order_total FROM orders, customers WHERE customer_id = customers.id AND email = 'pclark74@gmail.com';
...
SELECT orders.id, status, order_total FROM orders JOIN customers ON customer_id = customers.id AND email = 'pclark74@gmail.com';
...
SELECT melons.common_name, melons.melon_type, order_items.quantity, order_items.unit_price, order_items.total_price FROM order_items JOIN melons ON melon_id = melons.id WHERE order_items.order_id = 2725;
